...
The official Casio F-91W user manual (Module 593) is included at the end of this document as reference material, which serves as the authoritative source on how the gossamer app should behave.
...
time modes (on short press release), long-pressing ALARM displays the "Dozenal Watch"
scrolling animation in the bottom row, and long-pressing ALARM does not display the year or date.
...
- **ALARM Button Function on Main Clock:**
  - *Original F-91W:* Pressing ALARM toggles between 12-hour and 24-hour display modes (showing a "24H" indicator in 24H mode). Holding ALARM displays "CASIO" across the screen as an authenticity test (Easter egg).
  - *Current UI:* Pressing or releasing ALARM cycles through all available time modes (`12H` -> `24H` -> `DIURNAL` -> `SEMIDIURNAL`). Pressing and holding ALARM displays a "Dozenal Watch" scrolling animation in the bottom row (Easter egg), returning to normal time display upon release. Holding ALARM does not display the year or date.
...
# Reference: Casio F-91W User Manual (Module 593)
...
* **Authenticity display ("CASIO" Easter Egg)**: In regular timekeeping mode, pressing and holding the **A** button for approximately 3 seconds causes **CASIO** to appear across the display. Releasing the button returns to the regular timekeeping display.
